"""
Real-Time Video Processing Script for Approach 2 - Traffic Light Detection
...
def calculate_display_size(frame_width, frame_height, max_width=1920, max_height=1080):
    """
...
def process_video_realtime(
    ...
    print("=" * 70)
    print("Approach 2: Real-Time Traffic Light Detection with Alerts")
    print("=" * 70)
    ...
    try:
        while True:
            ...
            if paused:
                if show_preview and last_processed_frame is not None:
                    ...
                    cv2.imshow('Real-Time Traffic Light Detection - Approach 2', display_frame)
        ...
        if processed_frames > 0:
            print(f"   Average per Processed Frame: {total_detections/processed_frames:.2f}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Real-time video processing with Approach 2 Traffic Light Detection'
    )
    parser.add_argument(
        '--input', '-i',
        type=str,
    ...
    parser.add_argument(
        '--output', '-o',
        type=str,
        default='output_realtime.mp4',
        help='Output video file path (default: output_realtime.mp4)'
    )
    parser.add_argument(
        '--conf', '-c',
        type=float,
